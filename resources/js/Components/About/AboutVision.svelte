<script>
  import { Compass, Anchor, Shield, TrendingUp, ArrowLeft, ArrowRight } from "lucide-svelte";
  import { inview } from "@/lib/utils";
  import { fly } from "svelte/transition";
  import { cubicOut, cubicIn } from "svelte/easing";
  import { t } from "@/lib/i18n";

  let show = false;

  const missionIcons = [Compass, Anchor, Shield, TrendingUp];

  let currentMissionIndex = 0;
  let direction = 'next';

  function nextMission() {
    direction = 'next';
    currentMissionIndex = (currentMissionIndex + 1) % $t.ABOUT_VISION_MISSIONS.length;
  }

  function prevMission() {
    direction = 'prev';
    currentMissionIndex = (currentMissionIndex - 1 + $t.ABOUT_VISION_MISSIONS.length) % $t.ABOUT_VISION_MISSIONS.length;
  }
</script>

<section class="relative py-24 md:py-32" use:inview on:enter={() => show = true}>
  <div class="max-w-[1200px] mx-auto px-[var(--gutter)]">
    {#if show}
      <div 
        transition:fly={{ y: 40, duration: 1000, delay: 100, easing: cubicOut }} 
        class="bg-[#fafafa] rounded-[32px] md:rounded-[48px] overflow-hidden flex flex-col lg:flex-row shadow-[0_30px_80px_rgba(0,0,0,0.5)] relative"
      >
        <!-- Subtle inner gradient for premium light feel -->
        <div class="absolute inset-0 bg-gradient-to-br from-white via-[#fafafa] to-[#f4f4f4] pointer-events-none z-0"></div>

        <!-- Left: Visi (Static) -->
        <div class="lg:w-1/2 p-8 md:p-14 lg:p-20 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-black/5 bg-transparent relative z-10">
          <div>
            <!-- Section Pill -->
            <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-xs font-semibold text-primary-light uppercase tracking-wider mb-10 shadow-[0_4px_15px_rgba(247,37,134,0.05)]">
              <Compass class="size-3.5 text-primary-light" />
              <span>{$t.ABOUT_VISION_BADGE}</span>
            </div>
            
            <h2 class="font-display text-4xl md:text-5xl lg:text-[52px] font-bold tracking-tight text-neutral-900 leading-[1.15] uppercase mb-8">
              {@html $t.ABOUT_VISION_TITLE}
            </h2>
            
            <p class="font-body text-neutral-500 text-base md:text-lg leading-relaxed max-w-md">
              {$t.ABOUT_VISION_DESC}
            </p>
          </div>
          
          <!-- Navigation Arrows -->
          <div class="flex items-center gap-3 mt-16">
            <button on:click={prevMission} class="w-12 h-12 rounded-full bg-black/5 hover:bg-black/10 flex items-center justify-center text-neutral-600 transition-colors">
              <ArrowLeft class="size-5" />
            </button>
            <button on:click={nextMission} class="w-12 h-12 rounded-full bg-primary hover:bg-primary/90 flex items-center justify-center text-white transition-colors shadow-lg shadow-primary/30">
              <ArrowRight class="size-5" />
            </button>
          </div>
        </div>

        <!-- Right: Misi (Dynamic Carousel) -->
        <div class="lg:w-1/2 p-4 md:p-10 lg:p-14 relative flex flex-col justify-center min-h-[400px] lg:min-h-[600px] z-10 overflow-hidden bg-transparent">
          
          {#key currentMissionIndex}
            <div 
              class="absolute inset-0 p-4 pb-12 md:p-10 md:pb-10 flex flex-col items-center justify-center"
              in:fly={{ x: direction === 'next' ? 300 : -300, duration: 800, opacity: 0, delay: 50, easing: cubicOut }}
              out:fly={{ x: direction === 'next' ? -300 : 300, duration: 800, opacity: 0, easing: cubicIn }}
            >
              <!-- Container Card -->
              <div class="w-full h-full bg-white rounded-[32px] md:rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-black/5 flex flex-col items-center justify-center text-center p-6 md:p-12 relative overflow-hidden">
              <!-- Icon -->
              <div class="w-16 h-16 rounded-full border border-primary/20 bg-primary/5 flex items-center justify-center text-primary mb-8 mx-auto shadow-inner">
                <svelte:component this={missionIcons[currentMissionIndex]} class="size-8" />
              </div>
              
              <!-- Mission Content -->
              <h3 class="font-display text-2xl md:text-3xl font-semibold text-neutral-900 mb-5">
                {$t.ABOUT_VISION_MISSIONS[currentMissionIndex].title}
              </h3>
              
                <p class="font-body text-neutral-500 text-lg leading-relaxed max-w-sm mx-auto">
                  {$t.ABOUT_VISION_MISSIONS[currentMissionIndex].desc}
                </p>
              </div>
            </div>
          {/key}
          
          <!-- Indicator -->
          <div class="absolute bottom-4 left-1/2 -translate-x-1/2 font-display text-sm font-bold text-primary/50 tracking-widest z-20">
            {currentMissionIndex + 1}/{$t.ABOUT_VISION_MISSIONS.length}
          </div>
          
        </div>
      </div>
    {/if}
  </div>
</section>
